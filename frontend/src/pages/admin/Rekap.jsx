...
export default function Rekap() {
  ...
  async function load() {
    ...
    try {
      ...
      const data = await apiRequest(`/api/reports/rekap?${params.toString()}`);
      setRows(data);
    } catch (err) {
  ...
  return (
    <div>
      <header className="mb-6">
        <h1 className="font-serif text-2xl text-ink">Rekap & Ekspor</h1>
        <p className="mt-1 text-sm text-ink-faint">Rekap kehadiran seluruh guru & pegawai.</p>
      </header>
      ...
      <div className="mb-5 flex flex-wrap items-end gap-4 border border-line bg-paper-soft p-4">
        ...
        <div>
          ...
          <select value={userId} onChange={(e) => setUserId(e.target.value)} className="mt-1 border border-line bg-white px-2 py-1.5 text-sm">
      ...
      {!loading && !error && rows.length === 0 && (
        <EmptyState title="Tidak ada data" description="Coba ubah rentang tanggal atau filter." />
      )}
            ...
            <tbody>
              {rows.map((r) => (
                <tr key={r.id} className="border-b border-line">
                  ...
                  <td className="py-2.5 pr-4 tabular-nums">{r.terlambat_menit > 0 ? `${r.terlambat_menit} mnt` : '-'}</td>
